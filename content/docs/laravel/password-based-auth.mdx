...
description: Password-based authentication block for Laravel Applications
---
...
<BlockPreview name="password-based-auth-laravel/auth/sign-up" />

## Installation

<BlockItem
  name="password-based-auth-laravel"
...
<RegistryBlock itemName="password-based-auth-laravel" />

## Usage

Once you install the block in your Laravel project, you'll get all the necessary pages and components to set up a password-based authentication flow. Laravel typically uses Inertia.js or Livewire for React integration.
...
<LoginForm
  onSignUpClick={() => router.visit("/sign-up")}
  onForgotPasswordClick={() => router.visit("/forgot-password")}
/>
); }`}
...
<SignUpForm
  ...
  onSuccess={() => router.visit("/protected")}
/>
); }`}
