...
description: Social authentication block for Astro applications
---
...
<RegistryBlock itemName="social-auth-astro" />

## Usage

Once you install the block in your Astro project, you'll get all the necessary pages and components to set up a social authentication flow using React islands.
...
For local development, add these to your `.env` (Convex will read them when you run `npx convex dev`):
...
2. **Add OAuth credentials to `.env`** - Convex reads environment variables for local development
...
1. Check that your `.env` file includes all required variables
