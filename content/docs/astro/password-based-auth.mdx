...
description: Password-based authentication block for Astro applications
---
...
<BlockItem
  name="password-based-auth-astro"
  description="All needed components for the password based auth flow"
/>

## Folder structure

This block includes the [Convex client](/ui/docs/astro/client). If you already have one installed, you can skip overwriting it.

<RegistryBlock itemName="password-based-auth-astro" />

## Usage

Once you install the block in your Astro project, you'll get all the necessary pages and components to set up a password-based authentication flow using React islands.
...
### Using auth components in Astro

Since authentication components require interactivity, they must be rendered as React islands with the `client:load` directive:
...
1. Create Astro pages for each auth route (login, sign-up, forgot-password, reset-password)

2. Update the redirect paths in `login-form.tsx` and `sign-up-form.tsx` components to point to the logged-in routes in your app. Our examples use `/protected`, but you can set this to whatever fits your app.

3. Add route protection to authenticated pages:
...
export default function LoginPage() {
  return (
    <LoginForm
      onSignUpClick={() => window.location.href = "/sign-up"}
...
export default function SignUpPage() {
  return (
    <SignUpForm
      ...
      onSuccess={() => window.location.href = "/protected"}
    />
  );
}`}
</CodeBlock>
...
If you get an error about `PUBLIC_CONVEX_URL` not being set:

1. Make sure you've run `npx convex dev` at least once
2. Check that `.env` exists in your project root
