...
This chat component uses **Convex real-time queries** to sync messages between connected clients.

Messages sent through Convex are:

- delivered in real time to all connected clients automatically
- **stored permanently** in your Convex database
- **guaranteed** to arrive (Convex handles retries and connection management)
- scoped by whatever query parameters you define (e.g., `roomName`)

This design gives you both real-time updates and persistent storage out of the box—no separate persistence layer needed.
