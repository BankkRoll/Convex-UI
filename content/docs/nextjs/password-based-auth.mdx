...
This block includes the [Convex client](/ui/docs/nextjs/client). If you already have one installed, you can skip overwriting it.
...
### Setting up proxy for route protection

The block includes a proxy file for protecting routes (Next.js 16+). Configure which routes require authentication:
...
export default convexAuthNextjsMiddleware({
  publicRoutes: ["/auth/login", "/auth/sign-up", "/auth/forgot-password"],
});
...
Update the redirect paths in `login-form.tsx` and `update-password-form.tsx` components to point to the logged-in routes in your app. Our examples use `/protected`, but you can set this to whatever fits your app.

<Callout type="info">

You can use this block with the Pages router by simply moving the routes from the `src/app` folder into the `pages` folder and renaming them. For example, instead of `src/app/sign-up/page.tsx`, you'd create a `pages/sign-up.tsx` file.
...
If you get an error about `NEXT_PUBLIC_CONVEX_URL` not being set:

1. Make sure you've run `npx convex dev` at least once
2. Check that `.env.local` exists in your project root
3. Restart your Next.js dev server after running `npx convex dev`
...
Make sure you've updated the redirect paths in `login-form.tsx` and `update-password-form.tsx` to match your app's protected route.
...
- [Next.js Proxy / Middleware](https://labs.convex.dev/auth/authz/nextjs)
