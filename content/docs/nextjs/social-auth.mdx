...
2. Visit `http://your-site-url/auth/login` to see this component in action.

<Callout type="info">

You can use this block with the Pages router by simply moving the routes from the `src/app` folder into the `pages` folder and renaming them. For example, instead of `src/app/login/page.tsx`, you'd create a `pages/login.tsx` file.
