...
<CodeBlock language="ts">{`import { defineSchema } from "convex/server";
...
<CodeBlock language="tsx">{`import { useConvexAuth } from "convex/react";
...
<CodeBlock language="tsx">{`import { LoginForm } from "@/components/auth/login-form";
import { useNavigate } from "@tanstack/react-router";
...
<CodeBlock language="tsx">{`import { SignUpForm } from "@/components/auth/sign-up-form";
import { useNavigate } from "@tanstack/react-router";
...
onSuccess={() => navigate({ to: "/protected" })}
/>
);
}`}</CodeBlock>
...
Make sure you've updated the redirect paths in `login.tsx` and `update-password.tsx` to match your app's protected route.
