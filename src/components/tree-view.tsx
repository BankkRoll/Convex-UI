...
>(
  (
    ...
    const titleText = trimmedDescription
      ? `${nameString}\n${trimmedDescription}`
      : nameString;
        ...
        <div className="flex items-center gap-x-3 truncate">
          {isBranch ? (
            <>
              {isLoading ? (
                <Loader2
                  className={cn("text-foreground-muted animate-spin")}
                  size={14}
                />
              ) : (
                <ChevronRight
...
const TreeViewFolderIcon = forwardRef<
  SVGSVGElement,
  LucideSVGProps & { isOpen?: boolean }
>(({ isOpen, ...props }, ref) => {
...
export { SQL_ICON, TreeView, TreeViewFolderIcon, TreeViewItem, flattenTree };
