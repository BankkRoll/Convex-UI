...
const components = {
  ...
  h5: ({ className, ...props }: React.HTMLAttributes<HTMLHeadingElement>) => (
    <h5
      className={cn("mt-8 scroll-m-20 text-lg tracking-tight", className)}
      {...props}
    />
  ),
  h6: ({ className, ...props }: React.HTMLAttributes<HTMLHeadingElement>) => (
    <h6
      className={cn("mt-8 scroll-m-20 text-base tracking-tight", className)}
      {...props}
    />
  ),
  ...
  ul: ({ className, ...props }: React.HTMLAttributes<HTMLUListElement>) => (
    <ul
      className={cn("my-6 ml-6 list-disc text-foreground-light", className)}
      {...props}
    />
  ),
  ol: ({ className, ...props }: React.HTMLAttributes<HTMLOListElement>) => (
    <ol
      className={cn("my-6 ml-6 list-decimal text-foreground-light", className)}
      {...props}
    />
  ),
  ...
  tr: ({ className, ...props }: React.HTMLAttributes<HTMLTableRowElement>) => (
    <tr
      className={cn("m-0 border-t p-0 even:bg-surface-75/75", className)}
      {...props}
    />
  ),
  ...
  // Figure wrapper for rehype-pretty-code
  figure: ({
    className,
    children,
    ...props
    ...
    const isCodeFigure =
      "data-rehype-pretty-code-figure" in (props as Record<string, unknown>);

    return (
      <figure className={cn(isCodeFigure && "group", className)} {...props}>
        {children}
      </figure>
    );
  },
